...
The Poisson stage starts from the three-dimensional SiMOS mesh. The device is
instantiated as an electron device,
\begin{lstlisting}
...
using a Fermi--Dirac approximation at
\[
    T = \SI{0.015}{\kelvin} = \SI{15}{\milli\kelvin}.
\]

The electrostatic biases are applied only through the surfaces declared with
\texttt{new\_gate\_bnd}. Each gate boundary receives the same work-function
offset,
\[
    \Phi_m = \chi_{\mathrm{Si}} + \frac{E_{g,\mathrm{Si}}}{2},
\]
corresponding to a mid-gap silicon reference. The voltages used in the
...
No ohmic boundary condition is imposed in this configuration. The reservoirs are
therefore included through their material regions and controlled
electrostatically by the reservoir gates listed above.
...
\]
This list does not introduce a new material; it selects part of the already
defined mesh. When it is passed to \texttt{d.set\_dot\_region}, the central
stack is kept out of the classical carrier-density calculation. This leaves the
dot to be treated as a few-electron quantum region rather than as a classical
electron gas that screens the gates.
